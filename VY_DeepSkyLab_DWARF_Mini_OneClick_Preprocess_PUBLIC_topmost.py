...
_STACK_N_RE = re.compile(r"stack[_\-]?(\d+)", re.IGNORECASE)
_GAIN_RE = re.compile(r"gain[_\-]?(\d+)", re.IGNORECASE)


def _parse_stack_n(filename: str) -> int:
    """Extract n from ...stack_n or ...stackn. Return 1 if not present."""
    m = _STACK_N_RE.search(filename)
    if m:
        try:
...
def _parse_gain_from_name(name: str) -> Optional[int]:
    m = _GAIN_RE.search(name)
    if m:
        try:
            return int(m.group(1))
        except ValueError:
...
def _select_best_calib_files(folder: Path, gain: int) -> List[Path]:
    """Select FITS files in folder prioritizing matching gain and highest stack_n."""
    files = _glob_fits(folder)
    if not files:
        return []

    # Filter for files matching gain if gain is specified in filename
    gain_matched = []
    for f in files:
        file_gain = _parse_gain_from_name(f.name)
        if file_gain is None or file_gain == gain:
            gain_matched.append(f)

    candidates = gain_matched if gain_matched else files

    # Find highest stack_n value
    max_n = max(_parse_stack_n(f.name) for f in candidates)
    best_files = [f for f in candidates if _parse_stack_n(f.name) == max_n]
...
def _select_matching_darks(dark_dir: Path, shots: ShotsInfo) -> List[Path]:
    ...
    matching_temp: List[Tuple[Path, DarkMeta]] = []
    if shots.min_temp is not None and shots.max_temp is not None:
        matching_temp = [(f, m) for (f, m) in candidates if shots.min_temp <= m.temp_c <= shots.max_temp]
    ...
    if not matching_temp:
        ...
        matching_temp = [(f, m) for (f, m) in candidates if abs(m.temp_c - target_t) == best_dist]

    # Pick the dark file(s) with the highest stack_n value for the matched temperature
    max_n = max(m.stack_n for (_, m) in matching_temp)
    best_darks = [f for (f, m) in matching_temp if m.stack_n == max_n]
    return sorted(best_darks)
...
def _center_tk_window(win, width=None, height=None) -> None:
...
def _tk_choose_astronomy_dir() -> Optional[Path]:
    ...
    root = tk.Tk()
    root.withdraw()
    _center_tk_window(root, width=600, height=400)
...
def main() -> None:
    ...
    if bias_src is not None:
        bias_files = _select_best_calib_files(bias_src, shots.gain)
    if not bias_files:
    ...
    if flat_src is not None:
        flat_files = _select_best_calib_files(flat_src, shots.gain)
        if flat_files:
    ...
    if has_dark:
        siril.log("Building master dark…")
        siril.cmd("cd", _cd_arg(dark_work))

        if len(dark_files) == 1:
            # Single frame selected (e.g. highest stack_n pre-stacked master dark)
            one = _glob_fits(dark_work)[0]
            _save_single_as_master(siril, one, master_dark)
            siril.log("Master dark: single frame / pre-stacked master → saved")
        else:
            # Build dark sequence from multiple frames in this folder
    ...
    if has_flat:
        ...
        if len(flat_files) == 1:
            # Single frame selected (e.g. highest stack_n pre-stacked master flat)
            one = _glob_fits(flat_work)[0]
            _save_single_as_master(siril, one, master_flat)
            siril.log("Master flat: single frame / pre-stacked master → saved")
        else:
    ...
    # frame is excluded and framing is left as the default "current".
    # '-framing=min' crops out non-overlapping edges but can fail with 'null or negative intersection'
    # on sequences with field rotation/drift (common in DWARF Alt-Az captures).
    # We try '-framing=min' first, falling back to '-framing=max' if min fails.
    framing_candidates = ["min", "max"]
    for fmode in framing_candidates:
        apply_args: List[str] = ["seqapplyreg", "pp_light", f"-framing={fmode}"]
        ...
        try:
            ...
            if fmode != framing_candidates[-1]:
                siril.log(f"Warning: seqapplyreg with -framing={fmode} failed ({err}). Retrying with -framing=max…")
            else:
                raise err
